...
if not os.path.exists(__location__+'/resources/programparts/sync/serverdatabase.txt'):
	with open(__location__+'/resources/programparts/sync/serverdatabase.txt', "a") as database:
		database.write("""##################################################################################################################
##The format is: ||salt||username||passwordHash||sizeLimit||
...
def newUserCommand(s):
	...
	with open(__location__+'/resources/programparts/sync/serverdatabase.txt', "rb") as database:
	...
	if not proper:
		...
		with open(__location__+'/resources/programparts/sync/serverdatabase.txt', "a") as database:
			database.write('||%s||%s||%s||%s||\n' % (salt,username,passwordHash,str(12800000000)))
...
def passwordMatch(username,passwordHash):
	match = False
	with open(__location__+'/resources/programparts/sync/serverdatabase.txt', "rb") as database:
		for line in database:
				if line.startswith('||'):
					if line.split('||')[2] == username:
						if line.split('||')[3] == passwordHash:
							match = True
							break
						else:
							match = False
